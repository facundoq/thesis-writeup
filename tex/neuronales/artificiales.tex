...
\begin{algorithm}[H]
\KwData{ \\Un conjunto de ejemplares $D \in \reals^d$, posiblemente con una clase una clase $y_i \in \{-1,1\}$ asociada a cada $\xi \in D$ \\
...
IMAGEN: d neuronas de entrada, una de salida, $o(\xv)$
...
\ma{ f(\xv) &= \theta_t(h(\xv)) \quad \text{donde:} \\
h(\xv) &= \wv \cdot \xv \\
